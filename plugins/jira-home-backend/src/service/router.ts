...
export async function createRouter(
  ...
  router.get('/project', async (_, res) => {
    logger.info(
      `Fetching Jira project details and issues for project key: ${projectKey}`,
    );
    ...
    try {
      ...
      if (!issuesResponse.ok) {
        ...
        logger.error(
          `Failed to fetch issues: ${issuesResponse.statusText}, Response: ${text}`,
        );
        return res
          .status(issuesResponse.status)
          .json({ error: issuesResponse.statusText });
      }
      ...
      if (!projectResponse.ok) {
        ...
        logger.error(
          `Failed to fetch project: ${projectResponse.statusText}, Response: ${text}`,
        );
        return res
          .status(projectResponse.status)
          .json({ error: projectResponse.statusText });
      }
      ...
      if (error instanceof Error) {
        logger.error(
          `Error fetching project details or issues: ${error.message}`,
        );
