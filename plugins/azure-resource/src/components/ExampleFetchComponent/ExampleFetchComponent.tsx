  ...
  const groupResources = (): GroupedResources => {
    ...
    resources.forEach((resource: AzureResource) => {
      if (resource.type === 'microsoft.network/publicipaddresses') {
        const networkInterfaceId =
          resource.properties?.ipConfiguration?.id.split(
            '/ipConfigurations',
          )[0];
    ...
    resources.forEach((resource: AzureResource) => {
      ...
      if (type === 'microsoft.compute/virtualmachines') {
        ...
        const associatedVM = grouped.virtualMachines.find(
          vm => vm.resourceGroup === resource.resourceGroup,
        );
        if (associatedVM) {
          associatedVM.properties.relatedResources =
            associatedVM.properties.relatedResources || [];
            ...
            {resource.type.toLowerCase().includes('virtualmachines') &&
              resource.properties && (
                <>
                  ...
                  <Typography variant="body1" style={{ color: '#aaa' }}>
                    <strong>Disk Size (GB):</strong>{' '}
                    {resource.properties.storageProfile.osDisk.diskSizeGB}
                  </Typography>
                  {resource.properties.relatedResources && (
                    <div style={{ marginTop: '10px' }}>
                      ...
                      {resource.properties.relatedResources.map(
                        (relatedResource: any, index: number) => (
                          <Typography
                            key={index}
                            variant="body2"
                            style={{ color: '#aaa' }}
    ...
    <div>
      ...
      <Grid container spacing={3}>
        {groupedResources.virtualMachines.length > 0 && (
          <Grid item xs={12}>
            {renderResourceBlock(
              groupedResources.virtualMachines,
              'Virtual Machines',
            )}
          </Grid>
        )}
        {groupedResources.storageAccounts.length > 0 && (
          <Grid item xs={12}>
            {renderResourceBlock(
              groupedResources.storageAccounts,
              'Storage Accounts',
            )}
          </Grid>
        )}
