...
describe('ExampleFetchComponent', () => {
  ...
  it('should render the loading state initially', async () => {
    mockAzureResourcesApi.getResourcesByTag.mockReturnValue(
      new Promise(() => {}),
    ); // Keep it pending to simulate loading state
  ...
  it('should handle errors while fetching resources', async () => {
    mockAzureResourcesApi.getResourcesByTag.mockRejectedValue(
    ...
    expect(console.error).toHaveBeenCalledWith(
      'Error fetching Azure resources:',
      expect.any(Error),
    );
  });
});
